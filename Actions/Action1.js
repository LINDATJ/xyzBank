...
class Action {
    ...
    dropdown(locator, logname) {
        ...
        protractor_1.browser.wait(protractor_1.ExpectedConditions.elementToBeClickable(ele), 5000).then(function () {
    ...
    myClick(locator, logname) {
        let ele = protractor_1.element(protractor_1.by.xpath(locator));
        protractor_1.browser.wait(protractor_1.ExpectedConditions.presenceOf(ele), 5000).then(function () {
            ele.click().then(function () {
                console.log("clicked" + logname);
            }, function (err) {
